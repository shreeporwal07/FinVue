...
function RealTimeGraph() {
  ...
  const sell = () => {
    if (gameActive && unitsBought > 0) {
      ...
      const profitFromTransaction = Number((sellValue - lastBuyValue).toFixed(2));
      setRealTimeProfitLoss(prevProfit => Number((prevProfit + profitFromTransaction).toFixed(2))); // Update real-time profit/loss
  ...
  return (
    <div className="flex flex-col col-span-full sm:col-span-6 bg-gradient-to-r from-[#434974] to-[#242949] shadow-lg rounded-lg border border-slate-200 dark:border-slate-700">
      <header className="px-5 py-4 border-b border-slate-100 dark:border-slate-700 flex items-center">
        <h2 className="font-semibold text-slate-800 dark:text-slate-100">Market Mastery Challenge</h2>
      ...
      <div className="px-5 py-3">
        {gameActive ? (
          <p className="text-white text-sm mb-2">
            Total Amount: ${userMoney.toFixed(2)} Time Left: {timeRemaining} sec
          </p>
        ) : (
          <p className="text-white text-sm mb-2">
            You have ${userMoney.toFixed(2)} and you have to make $30 profit in {timeRemaining} sec
          </p>
        )}
        {!gameActive && (
        ...
        {gameActive && (
          <>
            ...
            <p className="text-white mt-2">Real-time Profit/Loss: ${realTimeProfitLoss.toFixed(2)}</p>
            <p className="text-white mt-2">Units Bought: {unitsBought}</p>
          </>
        )}
        {timeRemaining === 0 && (
          <p className="text-white mt-4">
            Game Over! Real-time Profit/Loss: ${realTimeProfitLoss.toFixed(2)}
          </p>
        )}
      </div>
    </div>
